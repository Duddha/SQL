CREATE OR REPLACE PACKAGE QWERTY.HR IS
...
END HR;
/
CREATE OR REPLACE PACKAGE BODY QWERTY.HR IS
  ...
  */
  ...
  BEGIN
    ...
    -- корпус
    IF (pADDRESS_REC.korp IS NULL) OR
       (TRIM(pADDRESS_REC.korp) = '0') OR
       (TRIM(pADDRESS_REC.korp) = '-') THEN
      korpus_str := '';
    ELSE
      korpus_str := ', корп. ' || pADDRESS_REC.korp;
    END IF;
    -- дом
    IF (TRIM(pADDRESS_REC.dom) = '0') OR
       (TRIM(pADDRESS_REC.dom) = '-') OR
       (TRIM(pADDRESS_REC.dom) IS NULL) THEN
      dom_str := '';
    ELSE
      IF pLANG = 1 THEN
    ...
    END IF;
    -- квартира
    IF (TRIM(pADDRESS_REC.kvart) = '0') OR
       (TRIM(pADDRESS_REC.kvart) = '00') OR
       (TRIM(pADDRESS_REC.kvart) = '-') OR
       (TRIM(pADDRESS_REC.kvart) IS NULL) THEN
      kvartira_str := '';
    ELSE
    ...
    -- улица
    IF pLANG = 1 THEN
      IF (pADDRESS_REC.name_line_u <> '0') AND
         (pADDRESS_REC.name_line_u <> '-') THEN
        RESULT := LOWER(line_rec.snam_u) || ' ' || pADDRESS_REC.name_line_u || dom_str || korpus_str || kvartira_str;
      END IF;
    ELSE
      IF (pADDRESS_REC.name_line_r <> '0') AND
         (pADDRESS_REC.name_line_r <> '-') THEN
