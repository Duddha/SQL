select pdr.name_u "Цех",
  ...
  from qwerty.sp_stat         st,
       ...
       (select id_tab,
       replace(
       ltrim(sys_connect_by_path(chr(13) || rn || ') ' || trim(education),
                                 ';;'),
             ';;' || chr(13)), ';;', ';') education,
  ...
  from (select id_tab,
          ...
          from (select obr.id_tab,
                       ...
                       decode(nvl(uz.name_u, ''), '', '', ', ' || uz.name_u ||
                              decode(nvl(uz.id_syti, 0), 0, '', ', ' || decode(pnkt.id, 28, '', pnkt.snam_u) || c.name_u)) --Учебное заведение
...
connect by prior education = prev_eductn
 ...
 where st.id_kat = 1
   and rbk.id_stat = st.id_stat
   and rbf.id_tab = rbk.id_tab
   and pdr.id_cex = st.id_cex
   and pdr.id_cex not in (8500, 8300, 7500, 7100)
