-- TAB = Инвалиды за год (для Бабкиной Юлии)

-- CHANGES:
--   01.02.2016: если групп инвалидности несколько, но они одинаковы, то второй раз группа не указывается
...
SELECT DISTINCT rbf.fam_u || ' ' || rbf.f_name_u || ' ' || rbf.s_name_u "ПІБ"
                ...
                ,TRIM(lgt.prim) "Група інвалідності"
       ...
       ,(SELECT id_tab
              ...
              ,decode(sign(dat_n - nvl(prev_finish_of_lgt
                     ...
                     ,decode(dat_n - prev_finish_of_lgt
                            ,1
                            ,''
                            ,'по ' || prev_finish_of_lgt || chr(13) || 'с ' || dat_n)) lgt_pause
          FROM (SELECT id_tab
                      ,id_lgt
                      ,srok
                      ,lead(srok) over(PARTITION BY id_tab ORDER BY dat_n) next_one
                      ,lag(srok) over(PARTITION BY id_tab ORDER BY dat_n) prev_one
                      -- Если группа инвалидности не меняется, тогда второй раз её не указываем
                      ,decode(name_rus
                             ,lag(name_rus) over(PARTITION BY id_tab ORDER BY dat_n)
                             ,''
                             ,name_rus) name_rus
