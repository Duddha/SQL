...
select '1' id_vid, --Для работников
       ...
       t.f_name_u f_name_u,
       t.s_name_u s_name_u,
       nvl2(osnn.id_nalog, osnn.id_nalog, to_char(o.id_nalog)) id_nalog,
       o.data_r dat_r,
  ...
  from QWERTY.SP_RBV_TAB_NAME t,
      ...
      ,
       (select *
         ...
         where rn = 1) adr
 where t.id_tab in
        &<name = "Список" list = "(select id_tab from qwerty.sp_ka_work minus select id_tab from qwerty.sp_ka_pens_all where id_tab not in (select id_tab from qwerty.sp_ka_lost where lost_type = 1) and kto <> 7), Работники завода,
                                   (select id_tab from qwerty.sp_ka_pens_all where id_tab not in (select id_tab from qwerty.sp_ka_lost where lost_type = 1) and kto <> 7), Пенсионеры завода" description = "yes">       
