...
--     эту ситуацию можно обойти при помощи JOIN (см. Вариант 3)
...
WITH get_rows1 AS
 (SELECT rownum row_number
    FROM dual
  CONNECT BY 1 = 1
         AND rownum <= &< NAME = "Количество строк для выборки" HINT = "Введите число строк, который вы хотите получить" TYPE = "integer" DEFAULT = "100" >)
...
SELECT column_value "№ п/п" FROM TABLE(CAST(MULTISET (SELECT LEVEL FROM dual CONNECT BY LEVEL <= &< NAME = "Количество строк для выборки" >) AS sys.odcinumberlist))
